...
export const UploaderProvider = ({ children }: { children: ReactNode }) => {
  ...
  const updateUploadStats = useCallback((progress: number, session: UploadSession) => {
    ...
    if (timeDiff > 1000 && progressDiff > 0) { // Update every second
  ...
  const getOptimalUploadSettings = useCallback((files: File[]): UploadOptions => {
    ...
    if (largestFile >= 500 * 1024 * 1024) { // 500MB+
      chunkSize = 20 * 1024 * 1024; // 10MB chunks
      maxConcurrency = 4;
    } else if (largestFile >= 100 * 1024 * 1024) { // 100MB+
      chunkSize = 15 * 1024 * 1024; // 8MB chunks
      maxConcurrency = 3;
    } else if (largestFile >= 50 * 1024 * 1024) { // 50MB+
  ...
  const uploadFile = useCallback((files: File[], options?: UploadOptions) => {
  ...
  const uploadFileChunked = useCallback((file: File, options?: UploadOptions) => {
    const chunkOptions = {
